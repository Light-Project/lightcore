...
{
    PIDC_CONTEXT(pctx, INT16_MAX * 0.6, INT16_MAX * 0.5, INT16_MAX * 0.125, 0, 1000);
    int32_t retval, curr = 100;
    unsigned int count = 0;

    for (count = 0; count < TEST_LOOP; ++count) {
        retval = pidc_update(&pctx, 1000, curr);
        curr += retval - 200;
        kshell_printf(ctx, "pidc static test%02d: %d\n", count, curr);
    }
    ...
    for (count = 0; count < TEST_LOOP; ++count) {
        ...
        kshell_printf(ctx, "pidc dynamic test%02d: %d\n", count, curr);
    }
