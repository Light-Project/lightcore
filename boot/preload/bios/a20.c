...
void a20_enable(void)
{
    ...
    while (count--) {
        if (a20_test(A20_TEST_FAST)) {
            pr_boot("A20 is already activated\n");
            return;
        }

        a20_bios();
        if (a20_test(A20_TEST_FAST)) {
            pr_boot("Enable A20 via BIOS\n");
            return;
        }

        a20_kbd();
        if (a20_test(A20_TEST_SLOW)) {
            pr_boot("Enable A20 via I8042\n");
            return;
        }

        a20_pch();
        if (a20_test(A20_TEST_SLOW)) {
            pr_boot("Enable A20 via PCH\n");
            return;
        }

        a20_port();
        if (a20_test(A20_TEST_SLOW)) {
            pr_boot("Enable A20 via IOPort\n");
            return;
        }
    }

    panic("Noway to enable A20\n");
}
