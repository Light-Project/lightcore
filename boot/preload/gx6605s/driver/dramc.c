...
static const uint32_t dramc_ctl_config[155] = {
...
static const uint32_t dramc_phy_config[26] = {
...
static const struct dramc_fixup dramc_fixup_table[] = {
    {  74,   0,  0x0001,  0x0001},
    {  77,  24,  0x0001,  0x0001},
    { 149,   8,  0x0001,  0x0001},
...
static void dramc_fill_config(void)
{
    unsigned int count;
    uint32_t val;
    ...
    for (count = 0; count < ARRAY_SIZE(dramc_ctl_config); count++) {
        ...
        writel(DRAMC_BASE + (count << 2), val);
    }

    for (count = 0; count < ARRAY_SIZE(dramc_phy_config); count++) {
        val = dramc_phy_config[count];
        writel(DRAMC_BASE + 0x400 + (count << 2), val);
    }
}

static void dramc_fixup_config(void)
{
    ...
    for (count = 0; count < ARRAY_SIZE(dramc_fixup_table); count++) {
        ...
        val = readl(DRAMC_BASE + (table->offset << 2));
        val &= ~(table->off << table->shift);
        val |= table->set << table->shift;
        writel(DRAMC_BASE + (table->offset << 2), val);
    }
}

static bool dramc_wait_ready(void)
{
    unsigned int timeout = 0xffffff;
    uint32_t val;

    while (--timeout) {
        val = readl(DRAMC_BASE + 0xb8);
        if (val & 0x20)
            break;
    }
...
void dramc_init(void)
{
    ...
    *(volatile uint32_t *)(GCTL_BASE + GX6605S_MPEG_CLD_RST1_DAT) &= ~(0x1);
    ...
    writel(GCTL_BASE + GX6605S_DRAM_CTRL0, 0x00);
    writel(GCTL_BASE + GX6605S_DRAM_CTRL1, 0x00);
    writel(GCTL_BASE + GX6605S_DRAM_CTRL2, 0x00);
...
# error "unknow dram type"
    ...
    *((volatile uint32_t *)(GCTL_BASE + GX6605S_DRAM_CTRL2)) &= ~(0xf << 16);
    ...
    if (memtest_fast(DRAM_BASE + (63 * SZ_1MiB), SZ_1KiB))
        panic("dram init error\n");
}
