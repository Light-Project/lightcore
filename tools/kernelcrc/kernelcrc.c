...
#include <stdint.h>
#include <stdarg.h>
#include "../../include/compiler/compiler_attributes.h"
...
static uint32_t crc32_byte(uint8_t c, uint32_t crc)
{
    return crc32_table[(crc ^ c) & 0xff] ^ (crc >> 8);
}

static uint32_t crc32(const uint8_t *s, int len, uint32_t crc)
{
    while (len--)
...
int main(int argc, char *argv[])
{
    ...
    fseek(kernel, SEEK_SET, SEEK_END);  
    kernel_size = ftell(kernel);
    
    buffer = malloc(kernel_size);
    memset(buffer, 0x00, kernel_size);
    
    fseek(kernel, 0, SEEK_SET);
    fread(buffer, kernel_size, 1, kernel);
    fclose(kernel);
    ...
    boot_head->crc = 0;
    new_crc = crc32(boot_head + 1, boot_head->size, 0xffffffff);
    if(old_crc != new_crc)
    {
