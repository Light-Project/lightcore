...
enum pgtbl_modified {
    __PGTBL_MODIFIED_PTE = 0,
    __PGTBL_MODIFIED_PMD,
    __PGTBL_MODIFIED_PUD,
    __PGTBL_MODIFIED_P4D,
    __PGTBL_MODIFIED_PGD,

    PGTBL_MODIFIED_PTE = BIT(__PGTBL_MODIFIED_PTE),
    PGTBL_MODIFIED_PMD = BIT(__PGTBL_MODIFIED_PMD),
    PGTBL_MODIFIED_PUD = BIT(__PGTBL_MODIFIED_PUD),
    PGTBL_MODIFIED_P4D = BIT(__PGTBL_MODIFIED_P4D),
    PGTBL_MODIFIED_PGD = BIT(__PGTBL_MODIFIED_PGD),
};

#ifndef pte_index
static inline uintptr_t pte_index(uintptr_t addr)
{
...
#ifndef pmd_index
static inline uintptr_t pmd_index(uintptr_t addr)
{
...
#ifndef pud_index
static inline uintptr_t pud_index(uintptr_t addr)
{
...
#ifndef p4d_index
static inline uintptr_t p4d_index(uintptr_t addr)
{
...
#ifndef pgd_index
static inline uintptr_t pgd_index(uintptr_t addr)
{
...
#ifndef pte_offset
static inline pte_t *pte_offset(pmd_t *pmd, uintptr_t addr)
{
...
#ifndef pgd_offset
static inline pgd_t *pgd_offset(pgd_t *pgd, uintptr_t addr)
{
...
{
    uintptr_t bound = (addr + PMD_SIZE) & PMD_MASK;
...
{
    uintptr_t bound = (addr + PUD_SHIFT) & PUD_MASK;
    return min(bound - addr, size);
}
#endif
...
{
    uintptr_t bound = (addr + P4D_SIZE) & P4D_MASK;
    return min(bound - addr, size);
}
#endif

#ifndef pgd_bound_size
static inline size_t pgd_bound_size(uintptr_t addr, size_t size)
{
    uintptr_t bound = (addr + PGDIR_SIZE) & PGDIR_MASK;
    return min(bound - addr, size);
}
#endif
